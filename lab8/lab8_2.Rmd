...
expenditures_tidy <- expenditures %>%
  pivot_longer(cols = starts_with('19'), names_to = 'Year', values_to = 'Observations') %>%
  rename(Observations = 'expenditure', 
         'Areas' = expenditure)
expenditures_tidy
```
...
```{r}
expenditures_tidy %>%
  select(Areas, Observations, Year) %>%
  filter(Areas == 'Medical and Health')
```
...
heartrate3 <- heartrate2 %>% 
  separate(patient, into=c("male", 'female'), sep = '_') # %>%
# pivot_longer(names_to = 'drug', values_to = 'heartrate')
...
```{r}
# unite(heartrate3, ('patient' & 'sex'), into=('patient'))
```
...
```{r}
beachbugs <- read.csv(here('lab8','data', 'sydneybeaches.csv'))
beachbugs
```
2. Use `pivot_wider` to transform the data into wide format.
```{r}
beachbugs %>%
  pivot_wider(names_from = Region, values_from = BeachId)
```
3. Now, use `pivot_longer` to transform them back to long!
```{r}
pivot_longer(beachbugs, cols = 'Region', values_to = 'BeachID')
```
