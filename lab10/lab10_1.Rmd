...
```{r}
elephants %>%
  ggplot(aes(x = age, y = sex)) + geom_count()
```
...
```{r}
homerange %>% 
  ggplot(aes(x = thermoregulation, fill = thermoregulation)) + geom_bar() + labs(title = '# Thermoregulaton type', x = 'Type', y = 'Count')
```
...
```{r}
#homerange %>%
 # ggplot(aes(x = log10.mass, y = homerange, size = log10.mass)) + geom_point()
```
